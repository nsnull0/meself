...
function polyfillsAreLoaded() {
  ...
  var tmp,
    ...
    TOKEN_MAPBOX =
      "pk.eyJ1IjoiZ3VuYXdhbndpamF5YSIsImEiOiJjajM2Zm54cngwNTM3MzNxNjdqOXcxNTZ2In0.U2aHrmrL5HEht8hX2I4FzA",
    URL_GOOGLE_MAPS =
      "https://www.google.co.id/maps/place/Japan,+〒143-0023+Tōkyō-to,+Ōta-ku,+Sannō,+1+Chome−２４−１+凛ｏｍｏｒｉ/@" +
      GEO.center.lat +
      "," +
      GEO.center.long +
      "," +
      ...
      "z/",
    URL_SLACK_WEBHOOK =
      "https://hooks.slack.com/services/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";
  w.el = [];
  ...
  w.modal = {
    ...
    invoke: function(data, onClose) {
      ...
      if (data.show && !one(".modal")) {
        ...
        modal.innerHTML +=
          '<h1 class="header">' +
          (data.header ? data.header : tmp.header) +
          "</h1>";
        modal.innerHTML +=
          '<div class="body">' + (data.body ? data.body : tmp.body) + "</div>";
        modal.innerHTML += '<button class="close"></button>';
        modal.className +=
          "modal " + (data.className ? data.className : tmp.className);
        d.body.appendChild(modal);
        on(one(".modal .close"), "click", function() {
          w.modal.close();
          "function" === typeof onClose ? onClose() : 0;
        });
  ...
  function mouseTrigger_HeroAni(e) {
    ...
    if (w.stackOf_HeroAni.length < 1) {
      w.stackOf_HeroAni = data;
      ani(w.el[".hero .title span"]);
    } else {
  ...
  // gallery nav clicked
  on(
    all(".sliding .modal-wrapper .close, .sliding a.ios-apps-icon"),
    "click",
    function(e) {
  ...
  function lazyGallery() {
    ...
    while (i--) {
      ...
      if (tmp.width > 0) {
        tmp.src =
          tmp.src.indexOf("data:image") == 0
            ? (tmp.src = JSON.parse(g[i].dataset.img)[0])
            : tmp.src;
      }
    }
  }
  lazyGallery();
  on(w, "hashchange", lazyGallery);
  // lazyload gallery
          ...
          attachments: [
            {
              ...
              author_icon:
                ...
                md5(el.email.value.trim().toLowerCase()),
